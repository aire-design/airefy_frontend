...
export default function HomeFeed({ initialArticles }: { initialArticles: Article[] }) {
  ...
  async function loadMore() {
    setLoading(true);
    try {
      const res = await getArticles(page + 1, PAGE_SIZE);
  ...
  return (
    <div>
      <ArticleList
        articles={articles}
        emptyMessage="No stories yet. Be the first to write one!"
      />
      {hasMore && (
